...
| --------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------- |
| egress-deny                 | `netpol.cilium.io/egress-deny="true"`                 | Denies all egress traffic from the selected pods.                         |
| egress-to-cloudflare        | `netpol.cilium.io/egress-to-cloudflare="true"`        | Allows egress traffic to Cloudflare public IPs.                           |
| egress-to-gotify            | `netpol.cilium.io/egress-to-gotify="true"`            | Allows egress traffic to the cluster's Gotify server.                     |
| egress-to-host              | `netpol.cilium.io/egress-to-host="true"`              | Allows egress traffic to the host's local network.                        |
| egress-to-intra             | `netpol.cilium.io/egress-to-intra="true"`             | Allows egress traffic to all pods inside the same namespace.              |
| egress-to-kube-apiserver    | `netpol.cilium.io/egress-to-kube-apiserver="true"`    | Allows egress traffic to the Kubernetes API server.                       |
| egress-to-kube-dns          | `netpol.cilium.io/egress-to-kube-dns="true"`          | Allows egress traffic to the cluster DNS service.                         |
| egress-to-outside-dns       | `netpol.cilium.io/egress-to-outside-dns="true"`       | Allows egress traffic to outside DNS servers |
| egress-to-public-ips        | `netpol.cilium.io/egress-to-public-ips="true"`        | Allows egress traffic to public IP addresses (excludes private ranges).   |
| egress-to-remote-node       | `netpol.cilium.io/egress-to-remote-node="true"`       | Allows egress traffic to other nodes' local network.                      |
| egress-to-world             | `netpol.cilium.io/egress-to-world="true"`             | Allows egress traffic to any destination outside the cluster (private IPs also). |
| ingress-deny                | `netpol.cilium.io/ingress-deny="true"`                | Denies all ingress traffic to the selected pods.                          |
| ingress-from-host           | `netpol.cilium.io/ingress-from-host="true"`           | Allows ingress traffic from host's local network.                         |
